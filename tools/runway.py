...
def pole(g):
    """杆子和底座。没建好的时候只有这一截"""
    rect(g, 18, 28, 4, 40, 'd')
    rect(g, 18, 28, 1, 40, 'D')            # 受光的那一边
    for y in range(33, 66, 6):              # 几道箍
        rect(g, 17, y, 6, 1, 'a')
    # 底座:**要有厚度**。一块平铺的板子读不出「立在地上」,
    # 得能看见顶面和侧面两层
    rect(g, 14, 68, 12, 2, 'D')            # 顶面(受光)
    rect(g, 13, 70, 14, 2, 'd')            # 侧面(背光,比顶面宽一点)
...
def unbuilt():
    ...
    rect(g, 14, 68, 12, 2, 'D')
    rect(g, 13, 70, 14, 2, 'd')
    rect(g, 16, 54, 8, 2, 'a')              # 杆头上一圈箍,像是等着往上接
    return outline(g)
