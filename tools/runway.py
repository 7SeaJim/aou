...
def base(g):
    ...
    **台子要够高。** 矮了它就只是甲板上鼓起来的一块,看不出是「站上去起飞」的
    地方;而老翘站在旗子边上、丫丫站在坡底下,这个高低差就是那两只鸟的关系。
    """
    rect(g, 16, 22, 38, 3, 'd')            # 台面下的横梁
    for x in (20, 33, 46):                 # 三根腿
        rect(g, x, 25, 3, 16, 'd')
        rect(g, x, 25, 1, 16, 'D')
    rect(g, 14, 40, 42, 2, 'd')            # 落在甲板上的底座
...
def ramp(g):
    ...
    for i in range(10):
        rect(g, 1 + i * 2, 40 - i * 2, 3, 1 + i * 2, 'D')
        rect(g, 1 + i * 2, 40 - i * 2, 3, 1, 'L')


def poles(g, flag=None):
    """两根杆。flag 给颜色就挂旗,不给就是两根空杆(还没建好)"""
    # 杆子在台面**两端**,旗子各自**朝外**飘。
    # 都朝里飘的话,两面旗正好盖住台面中间那段 —— 而那正是老翘要站的地方,
    # 于是「鸟站上去把旗挡没了」和「旗把鸟盖住了」轮流发生。朝外就都让开了。
    for x, dir in ((17, -1), (51, 1)):
        rect(g, x, 4, 2, 15, 'a')
        rect(g, x, 4, 1, 15, 'A')
        rect(g, x - 1, 3, 4, 1, 'K')
        if flag:
            # 旗子要**大、要带边**。小一号在缩略图上只剩几个红点,
            # 而旗正是「这儿建好了」的唯一记号
            for j, w in enumerate([9, 10, 9, 7, 4]):
                x0 = x + 1 if dir > 0 else x - w - 1
                rect(g, x0, 4 + j, w + 2, 1, 'K')
            for j, w in enumerate([9, 10, 9, 7, 4]):
                x0 = x + 2 if dir > 0 else x - w
                rect(g, x0, 4 + j, w, 1, flag)
                if j == 1:
                    rect(g, x0 + (1 if dir > 0 else w - 4), 5, 3, 1, 'R')
