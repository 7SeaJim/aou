...
def head_with_ears():
    ...
    **画在一起,只描一圈外边。** 脑袋描一圈、每只耳朵各描一圈的话,
    ...
    # 又高又大的一对立在脑门正上方,那是老鼠
    for cx in (3, 11):
        for j, w in enumerate([2, 3, 4, 4]):
            rect(h, cx - w // 2, 1 + j, w, 1, 'Y')
        rect(h, cx, 2, 1, 3, 'o')          # 耳朵里头
    return outline(h)
...
def sleeping():
    ...
    **就是两个椭圆**:小的那个是脑袋(连着耳朵),大的那个是身子,
    ...
    for x, y, h in ((17, 7, 4), (21, 7, 4), (25, 8, 3), (28, 10, 3)):
        rect(g, x, y, 1, h, 't')
    # 脑袋(连耳朵)整个压上来
    # 往下压三格,让下巴**搭在前爪和地板那条线上** ——
    # 悬在半空的脑袋不是在睡,是在发呆
    stamp(g, head_with_ears(), 0, 9)
    rect(g, 5, 13, 1, 2, 't'); rect(g, 9, 13, 1, 2, 't')     # 额头的纹
    # 肚皮和折在身下的前爪
    # **往右让开脸。** 原来肚皮和前爪一直铺到 x=11,而脑袋右边到 x=13 ——
    # 白的直接盖在脸上,脸和肚子糊成一块。中间空出来的那两三格身子色,
    # 就是「脸在前、肚子在后」这句话
    blob(g, 20, 19, 5, 2, 'w')
    rect(g, 15, 18, 4, 3, 'w')
    # 脸:两道闭着的眼 + 一个鼻头。**不画嘴** ——
