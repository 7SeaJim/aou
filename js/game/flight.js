 ...
 */
/**
 * ⚠️ **下面这三个道具的秒数,在 data.js 的 `ITEMS` 里有一份人话版本。**
 * 玩家在背包里读到的是那一份 —— **改了这儿就得回去改那儿。**
 *
 * 这条不是空话:护盾从 45 秒改到 12 秒之后,背包里整整挂了一版的「45 秒内挡两次」,
 * 是他自己看出来报过来的。玩家没法读源码,他只能读那一行;那一行错了,
 * 道具对他来说就是错的。
 *
 * (同一类耦合在这个仓库里还有一处:`SERVICE.span` 是 `serviceOpen()` 的人话版本。)
 */
 ...
 * **这个 1.4 是定过的,别顺手往回调。**
 *
 * 我当时把账摆出来问过他:十分钟那一档要吃到场上飘过的 71%(1.94 份/秒)
 * 才维持得住,而同时还得躲 —— 提了个 1.2 的软档(61%)当备选。
 * 他的回话是四个字:**「相信玩家」**。
 *
 * 所以这条线是有意画在那儿的:**一局跑到第十分钟,该是肚子把人送回巢,
 * 不是障碍。** 那时候速度、间距、簇宽早就封顶了(见八千米那一节),
 * 唯一还在涨的就是这条 —— 它就是这个玩法的终点线。
 *
 * 把它调软的结果不是「难度低一点」,是**第十分钟之后没有终点**:
 * 一个手熟的玩家可以一直飞下去,而那正是当初做饿这件事要解决的问题。
