...
const MAX_VY = 11.6;
/**
 * **往下掉的时候重力小一点(0.51 倍),终速也低一档。**
 *
 * 他说「下降速度稍慢一些」,后来又说了一次「再慢一些」。上升那一半不能动 ——
 * 一跳的高度和到顶的时间是整套关卡的地基(一跳正好一条道、横向只走 38 像素)。
 * 所以拆开:**顶上去用原来的重力,掉下来用小一点的。**
 *
 *     掉一条道   0.22 秒 → 0.26 秒 → 0.28 秒(实测)
 *     终速       696 px/s → 510 px/s → 396 px/s
 *
 * 不对称的重力是老办法,只是常见的方向反过来(通常是「掉得更快」让手感更脆)。
 * 这里要的是相反的东西:**掉得慢一点,就有时间在半空改主意** ——
 * 而这一版的密度和四条道,考的正是改主意。
 *
 * **能慢到哪儿是有底的,底在 `CLIMB_PX_PER_SEC`。** 那个 150 是算「下一簇的
 * 空道最远可以离多远」用的保守估计,当初就是按「松手掉到终速 6.5」写的 ——
 * 现在的 6.6 正好压在那条线上,再慢下去这个估计就变成高估,
 * 而高估等于**允许一次够不着的换道**。真要再慢,得连着把 CLIMB_PX_PER_SEC 一起降,
 * 那是在改关卡而不是改手感了。
 *
 * 对着最挤的情况验过一遍:两簇之间最短 520 毫秒,而门最多挪 150×0.52 = 78 像素,
 * 实测掉 78 像素要 0.333 秒 —— 还剩 0.19 秒余量。掉满两条道 0.45 秒也还在窗口里,
 * 而那本来就不该被逼出来(相邻两簇必有共用空道)。
 */
const FALL_G = 0.38;
const MAX_VY_DOWN = 6.6;
