...
export const EVENTS = [
    ...
    // 权重从 4 降到 2(占比 4.8% → 2.4%)。**道具换成长效小加成之后,
    // 出得勤就等于常驻** —— 而常驻的加成不是加成,是新的基线
    { id: 'toolbox', w: 2, name: '游客落下的东西',
 ...
 */
/**
 * 觅食带的三样小加成。**出发前自己挑要不要用,挑了才扣。**
 *
 * ------------------------------------------------------------
 * 这一版把道具整个换了性质。原来那三样(护盾/磁铁/金币)是**爆发型**的:
 * 十二秒挡两次、十五秒吸整屏、八秒无敌前冲 —— 和场上捡到的一模一样,
 * 而且**开局自动用掉**,玩家连"这局要不要带"都没得选。
 *
 * 他要的是另一种东西:**弱、长效、出发前自己挑**。
 *
 *     铁盾牌  下次觅食抵挡一次障碍      —— 一次,不限时,用掉才没
 *     吸铁石  觅食时吸取范围 +10%       —— 一整局都在,但只多一点点
 *     钱钱!   觅食时带回的资源 ×1.2     —— 同上
 *
 * > **两种道具的分工这样才清楚:场上捡的是爆发,背包带的是底子。**
 * > 原来两边是同一套效果,背包那份就只是"提前捡到一个",没有自己的位置。
 *
 * 键名和场上道具(`POWERUPS` 的 shield/magnet/double)**特意错开** ——
 * 它们现在是两回事,同名迟早会有人接错。老存档的三样按 1:1 换过来(v15→v16)。
 * ------------------------------------------------------------
 *
 * ⚠️ 效果的实现在 flight.js 的 `start()`(guard / foodR / haul 三个字段),
 * 消耗在 main.js 起飞那一步 —— **只扣玩家勾了的那几样。**
 */
export const ITEMS = {
    ironshield:  { name: '铁盾牌', icon: 'shield', desc: '下次觅食抵挡一次障碍' },
    magnetstone: { name: '吸铁石', icon: 'magnet', desc: '觅食时吸取范围 +10%' },
    moneybag:    { name: '钱钱!',  icon: 'double', desc: '觅食时带回的资源 ×1.2' },
};

/** 各自的系数。改这儿就改了效果,不用去翻 flight.js */
export const ITEM_EFFECT = {
    ironshield:  { guard: 1 },      // 能白挡几次
    magnetstone: { foodR: 1.10 },   // 吸取半径乘多少
    moneybag:    { haul: 1.20 },    // 带回去的资源乘多少
};
/**
