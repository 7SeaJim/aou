 ...
 */
/**
 * 觅食带的三样道具。**开局自动用掉一个,一局有效**(见 main.js 的扣除)。
 *
 * ⚠️ **这三行的数必须和 flight.js 里的常量对上。** 他报「背包里的道具文本
 * 没有根据飞行玩法进行修改」—— 对了一遍,护盾那条写的是 45 秒,
 * 而 `SHIELD_MS` 早就改成 12 秒了,差了近四倍;金币那条漏了「撞碎障碍」,
 * 而那是这一版专门给冲刺加的反馈(光「无敌」的那几秒玩家什么都感觉不到)。
 *
 * > **数值改了,说明它的那句话就是这次改动的一部分,不是「回头再说」。**
 * > 玩家没法读源码,他只能读这一行 —— 这一行错了,道具就是错的。
 *
 * 对应关系(改常量记得回来改这儿):
 *
 *     shield  SHIELD_MS 12 秒 · SHIELD_N 挡 2 次
 *     magnet  MAGNET_MS 15 秒 · 整屏吸(不是身边一圈)
 *     double  RUSH_MS 8 秒 · 无敌 + 前冲 + 撞碎障碍加分 + 食材翻倍
 *
 * 飞行中再捡到同样的会往上叠(各有上限),那是场上的事,这儿不写 ——
 * 背包这一行说的是「带一个进去能得到什么」。
 */
export const ITEMS = {
    shield: { name: '护盾', icon: 'shield', desc: '12 秒内挡两次' },
    magnet: { name: '磁铁', icon: 'magnet', desc: '15 秒吸走整屏食材' },
    double: { name: '金币', icon: 'double', desc: '8 秒无敌前冲,撞碎障碍,食材翻倍' },
};
